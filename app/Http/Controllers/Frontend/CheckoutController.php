...
class CheckoutController extends Controller
{
    ...
    public function complete(Request $request)
    {
        ...
        if (count($carts) > 0) {
            ...
            $total_points = 0;
            foreach ($carts as $cart) {
                // Ceci permet de savoir quel entreprise a vendu tel ou tel produit
                $product = Product::where('id',$cart->product_variation_id)->first();


                $orderItem                       = new OrderItem;
                $orderItem->order_id             = $order->id;
                $orderItem->shop_id             = $product->shop_id;
